...
double Astarpath::getHeu(MappingNodePtr node1, MappingNodePtr node2) {
  ...
  heu = sqrt(dx * dx + dy * dy + dz * dz);
    ...
    // tie breaker 用于避免路径在代价相同时震荡 use Washington distance as tie breaker
  tie_breaker = 0.001 * (dx + dy + dz);
    
  return heu * (1.0 + tie_breaker);
}
...
terminatePtr=terminatePtr->Father;
   ...
   *
   * **/
  // 将 front_path 中的坐标添加到 path
  for (const auto& nodePtr : front_path) {
    path.push_back(nodePtr->coord);
  }
