                      ...
                      Quadrotor::InternalState& dxdt, const double /* t */)
{
  ...
  motor_rpm_sq = cur_state.motor_rpm.array().square();    //电机转速的平方
  ...
  double resistance = 0.1 * 3.14159265 * (arm_length_) * (arm_length_) * cur_state.v.norm() * cur_state.v.norm();//resistance
  ...
  //通过外力，阻力计算
  // 更新 v_dot
  v_dot = (thrust * R.col(2) - mass_ * g_ * Eigen::Vector3d(0, 0, 1) - resistance * vnorm) / mass_ + external_force_/mass_;
  ...
  // 更新 omega_dot
  // 计算角动量
  Eigen::Vector3d angular_momentum = J_ * cur_state.omega;

  // 计算科里奥利力项
  Eigen::Vector3d coriolis = cur_state.omega.cross(angular_momentum);

  // 计算角加速度
  omega_dot = J_.inverse() * ( external_moment_ + moments - coriolis);
