...
private:
  ...
  Eigen::Vector3d des_pos_, des_vel_, des_acc_, kx_, kv_, kOmega_;
  double          des_yaw_, des_yaw_dot_;
  double          current_yaw_;
  bool            enable_motors_;
  bool            use_external_yaw_;
  double          kR_[3], kOm_[3], corrections_[3];
  double max_angular_rate_;  // 最大角速度限制
};
...
SO3ControlNodelet::publishSO3Command(void)
{
  ...
  const Eigen::Vector3d&    torque = controller_.getComputedTorque();
  ...
  const quadrotor_msgs::PositionCommand::ConstPtr& cmd)
{
  ...
  kx_ = Eigen::Vector3d(15, 10, 13);
  kv_ = Eigen::Vector3d(5, 10, 5);
  kOmega_ = Eigen::Vector3d(3, 5, 2);  // 角速度增益
  max_angular_rate_ = 0.001;  // 最大角速度限制
...
SO3ControlNodelet::odom_callback(const nav_msgs::Odometry::ConstPtr& odom)
{
  ...
  Eigen::Quaterniond attitude(odom->pose.pose.orientation.w,
                            odom->pose.pose.orientation.x,
                            odom->pose.pose.orientation.y,
                            odom->pose.pose.orientation.z);

  Eigen::Vector3d angular_vel(odom->twist.twist.angular.x,
                            odom->twist.twist.angular.y,
                            odom->twist.twist.angular.z);

  controller_.setPosition(position);
  controller_.setVelocity(velocity);
  controller_.setAttitude(attitude);
  controller_.setAngularVelocity(angular_vel);
  controller_.setAttitudeGains(kR_, kOm_);
  controller_.setMaxAngularRate(max_angular_rate_);  // 设置最大角速度

  current_yaw_ = tf::getYaw(odom->pose.pose.orientation);

  if (position_cmd_init_ && !position_cmd_updated_)
  {
    publishSO3Command();
  }
  position_cmd_updated_ = false;
